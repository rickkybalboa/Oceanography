...
AvgPlotGLOBENV(ctd.avg,geodiaH.sal,geodiaH.temp, geodiaP.sal, geodiaP.temp, xlimits, ylimits)
# 
...
png(file=paste0("~/Oceanography/Sponge_Project/Data/",region,"/RegionalPlot_",region,"_GLOBENV.png"),
...
for (f in files) {
  ...
  if (f == files[1]) { 
    ...
    title(paste0(region," Region TSPlot GLOBENV DATA"))
  } else { 
    par(new=T)
    plot(s,t
         , xlab = ''
