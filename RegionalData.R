...
geodia.hent <- na.omit(read.csv(paste0("./",region,"/Geodia_",region,"_Hentscheli.csv"), 
                                stringsAsFactors = FALSE))
geodia.parv <- na.omit(read.csv(paste0("./",region,"/Geodia_",region,"_Parva.csv"), 
                                stringsAsFactors = FALSE))

geodia <- rbind(geodia.hent,geodia.parv)
...
# Trim stations with < 100 data points from list
row_lt <- which(sapply(ctd, nrow) < 100)
if (length(row_lt) == 0) {  # necessary or it will cut all data on iterate
  print("File has >= 100 data points.")
} else {
  ctd <- ctd[-row_lt]
}; rm(row_lt)
files <- names(ctd)
...
geodiaH.temp <- geodia.hent[[grep("TEMP", names(geodia),ignore.case = TRUE)]]
geodiaH.sal <- geodia.hent[[grep("SAL", names(geodia), ignore.case = TRUE)]]
